...
//有噪音，经对比，原因
//1.线路有电流噪音 (两种mic比较下来，噪音程度差不多！)
//2.采集数据到发送有延时，造成声音不连续导致
...
void onWebSocketEvent(uint8_t client_num,
  ...
  //如果在供给一个函数mic输出时， 其它用户是进入不了的！
  int32_t low_sound = 10;   //声音下限 低于此值的声音数据截掉
  int32_t high_sound = 5000; //声音上限 高于此值的声音数据截掉

  //t-watch2020的mic是PDM,的I2S的不一样， 用40 听不到声音，可能是声音数据加倍后超过最大值造成，
  //建议用值: 10，20
  int add_sound = 15; //音量加倍系数
